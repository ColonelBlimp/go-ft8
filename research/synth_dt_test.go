...
func TestSyntheticDT(t *testing.T) {
	...
	duration := float64(NN) * float64(NSPS) / Fs // ~12.64s

	startSample := int(dtTarget * Fs) // 12000
	endSample := startSample + int(duration*Fs)
	if endSample > NMAX {
		endSample = NMAX
	}
	...
	expectedSample := int(dtTarget * Fs2) // 1.0 * 200 = 200
	expectedEnd := int((dtTarget + duration) * Fs2)

	t.Logf("")
	t.Logf("Downsampled signal analysis:")
	t.Logf("  cd0 length: %d samples (%.1f seconds)", len(cd0), float64(len(cd0))/Fs2)
	t.Logf("  Expected signal start:  sample %d (t=%.3f s)", expectedSample, float64(expectedSample)/Fs2)
	t.Logf("  Expected signal end:    sample %d (t=%.3f s)", expectedEnd, float64(expectedEnd)/Fs2)
	t.Logf("  Actual first energy:    sample %d (t=%.3f s)", firstSig, float64(firstSig)/Fs2)
	t.Logf("  Peak energy:            sample %d (t=%.3f s)", peakIdx, float64(peakIdx)/Fs2)

	offset := firstSig - expectedSample
	t.Logf("  Offset (first energy - expected): %d samples = %.3f seconds",
		offset, float64(offset)/Fs2)

	// Show energy distribution in 0.5s windows
	t.Logf("")
	t.Logf("Energy distribution (0.5s windows):")
	windowSize := int(0.5 * Fs2) // 100 samples
	for w := 0; w*windowSize < len(cd0) && w < 32; w++ {
		...
		t.Logf("  %5.1f-%5.1fs: energy=%.4e %s",
			float64(start)/Fs2, float64(end)/Fs2, energy, bar)
	}
	...
	t.Logf("  Best sync8d: ibest=%d (xdt=%.3f s), sync=%.4f",
		bestIdt, float64(bestIdt-1)*Dt2, bestSync)
...
func TestDownsampleTimingPulse(t *testing.T) {
	...
	expectedSample := int(1.0 * Fs2) // 200
	offset := peakIdx - expectedSample

	t.Logf("Pulse at t=1.000s (audio sample %d):", pulseCenter)
	t.Logf("  Expected in downsampled: sample %d (t=%.3f s)", expectedSample, float64(expectedSample)/Fs2)
	t.Logf("  Actual peak:             sample %d (t=%.3f s)", peakIdx, float64(peakIdx)/Fs2)
	t.Logf("  Offset: %d samples = %.3f seconds", offset, float64(offset)/Fs2)
	...
	for i := expectedSample - 10; i <= expectedSample+210; i += 5 {
		if i >= 0 && i < len(cd0) {
			...
			t.Logf("  sample %4d (t=%6.3fs): power=%.4e%s", i, float64(i)/Fs2, power, marker)
		}
	}
}
